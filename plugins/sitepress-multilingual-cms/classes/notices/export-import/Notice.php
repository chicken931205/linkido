...
class Notice implements \IWPML_Backend_Action, \IWPML_DIC_Action {
	...
	// WP Import Export removes and hides admin notices: override it!
	const WP_IMPORT_EXPORT_PRIORITY = 10001;
	...
	public function add_hooks() {
		...
		add_action( 'admin_head', [ $this, 'enforceNoticeStyle' ], self::WP_IMPORT_EXPORT_PRIORITY );
		add_action( 'admin_notices', [ $this, 'manageNotice' ], self::PRIORITY );
	}
	...
	private function maybeAddNotice( $id, $path, $message ) {
		...
		if ( $this->requiresWcmlActionButton( $id ) ) {
			$notice->add_action( $this->getWcmlActionButton() );
		}
		$notice->add_action( $this->getWpmlImportActionButton() );
	...
	public function enforceNoticeStyle() {
		...
		// WP Import Export removes and hides admin notices in its admin pages: enforce them.
		if ( false === has_action( 'admin_notices', [ $this, 'manageNotice' ] ) ) {
			add_action( 'admin_notices', [ $this, 'manageNotice' ], self::PRIORITY );
		}
	}
	...
	private function getWpmlImportLink() {
		...
		return sprintf(
			/* translators: %1$s and %2$s are opening and closing HTML tags for link. */
			__( '%1$sWPML Export and Import%2$s', 'sitepress' ),
	...
	private function getWcmlLink() {
		...
		return sprintf(
			/* translators: %1$s and %2$s are opening and closing HTML tags for link. */
			__( '%1$sWooCommerce Multilingual%2$s', 'sitepress' ),
			'<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '">',
			'</a>'
		);
	}
	 ...
	 */
	private function getWpmlImportActionButton() {
		$action = new \WPML_Notice_Action(
			__( 'Get WPML Export and Import', 'sitepress' ),
			self::WPML_IMPORT_URL,
			false,
			false,
			true,
			false
		);
		$action->set_link_target( '_blank' );
		return $action;
	}
	 ...
	 * @return bool
	 */
	private function requiresWcmlActionButton( $id ) {
		if ( ! $this->isNoticeForShop( $id ) ) {
			return false;
		}
	 ...
	 * @return \WPML_Notice_Action
	 */
	private function getWcmlActionButton() {
		$action = new \WPML_Notice_Action(
			__( 'Get WooCommerce Multilingual', 'sitepress' ),
			self::WCML_URL,
			false,
			false,
			true,
